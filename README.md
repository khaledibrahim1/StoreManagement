...
* **`DatabaseHelper.cs`**:
  * هو المسؤول الأول عن إدارة قاعدة بيانات **SQLite** المسماة (`store.db`).
  * عند تشغيل التطبيق لأول مرة، يقوم بإنشاء الملف والجداول الثلاثة (`Products`, `Transactions`, `TransactionItems`) إذا لم تكن موجودة.
...
* **`LoginViewModel.cs`**: يتحقق من بيانات تسجيل الدخول (اسم المستخدم الافتراضي: `admin` والرقم السري: `admin`) ويعرض رسالة خطأ في حال عدم تطابقها.
...
* **`LoginWindow.xaml`**: واجهة تسجيل دخول مصممة بأسلوب عصري يحتوي على حقول اسم المستخدم وكلمة المرور المشفرة.
